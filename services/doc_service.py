...
class DocService:
    ...
    def get_project_documents(project_id):
        with DatabasePool.get_connection() as conn:
            docs = conn.execute(DatabasePool.format_sql('SELECT * FROM project_documents WHERE project_id = ? ORDER BY upload_at DESC'), 
                               (project_id,)).fetchall()
    ...
    def delete_document(doc_id):
        with DatabasePool.get_connection() as conn:
            doc = conn.execute(DatabasePool.format_sql('SELECT file_path FROM project_documents WHERE id = ?'), (doc_id,)).fetchone()
            ...
            conn.execute(DatabasePool.format_sql('DELETE FROM project_documents WHERE id = ?'), (doc_id,))
            conn.commit()
            return True
    ...
    def get_document_info(doc_id):
        with DatabasePool.get_connection() as conn:
            doc = conn.execute(DatabasePool.format_sql('SELECT * FROM project_documents WHERE id = ?'), (doc_id,)).fetchone()
    ...
    def get_project_expenses(project_id):
        with DatabasePool.get_connection() as conn:
            expenses = conn.execute(DatabasePool.format_sql('SELECT * FROM project_expenses WHERE project_id = ? ORDER BY expense_date DESC'), 
                                   (project_id,)).fetchall()
    ...
    def update_expense(expense_id, data):
        with DatabasePool.get_connection() as conn:
            existing = conn.execute(
                DatabasePool.format_sql('SELECT * FROM project_expenses WHERE id = ?'),
                (expense_id,)
            ).fetchone()
            if not existing:
                return False
            ...
            approved_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S') if new_status == '已报销' else existing.get('approved_at')
            conn.execute(DatabasePool.format_sql('''
            ...
            '''), (
                data.get('expense_date', existing.get('expense_date')),
                data.get('expense_type', existing.get('expense_type')),
                data.get('amount', existing.get('amount')),
                data.get('description', existing.get('description')),
                data.get('applicant', existing.get('applicant')),
                new_status,
    ...
    def delete_expense(expense_id):
        with DatabasePool.get_connection() as conn:
            conn.execute(DatabasePool.format_sql('DELETE FROM project_expenses WHERE id = ?'), (expense_id,))
            conn.commit()
            return True
    ...
    def get_expense_stats(project_id):
        with DatabasePool.get_connection() as conn:
            total = conn.execute(DatabasePool.format_sql('SELECT SUM(amount) as total FROM project_expenses WHERE project_id = ?'), 
            ...
            by_status = conn.execute(DatabasePool.format_sql('''
                SELECT status, SUM(amount) as amount FROM project_expenses 
                WHERE project_id = ? GROUP BY status
            '''), (project_id,)).fetchall()
            
            by_month_sql = '''
                SELECT SUBSTR(CAST(expense_date AS TEXT), 1, 7) as month, SUM(amount) as amount
